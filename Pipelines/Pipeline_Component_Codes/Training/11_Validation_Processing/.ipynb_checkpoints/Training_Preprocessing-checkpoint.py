...
def preprocessing_function():
    ...
    ## Adding arguments
    # Inputs
    parser.add_argument('--full_data_location', type=str, default="/opt/ml/processing/input/data/churn-bigml-20.csv")
    ...
    try:
        ...
        whole_data["Churn"] = whole_data["Churn"].replace({False:0,True:1})
        
        # whole_data.to_csv(f"{output_location}/evaluation.csv", index=False)
        whole_data.to_csv("/opt/ml/processing/test/evaluation.csv", index=False)
